...
export default function List() {
    ...
    function tdgool(i){
        ...
        console.log(!copy[i].active)
        setTest(!copy[i].active)
                ...
                <ul className="tag_list">
                    {
                        test.map((item, i)=>{
                            ...
                            return(
                                <li className={ item.active ? 'on' : null } key={i} onClick={(e)=>{
                                    console.log(e.target.className)
                                    if(e.target.className == 'on'){
                                        e.target.className.add('on')
                                    }else{
                                        e.target.className.remove('on')
                                    }
                                    e.target.className == 'on' ? e.target.className.add('on') :  e.target.className.remove('on')
                                }} >{item.tag}</li>
                            )
                        })
                    }
